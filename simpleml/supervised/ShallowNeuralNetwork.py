...
class ShallowNeuralNetwork:
    ...
    def __init__(self, alpha=1e-5, max_iter=1e4, tol=1e-3, 
                 n_hid=25, lambd=0, 
    ...
    def fit(self, X, y):
        ...
        self.W1 = np.random.randn(n_in, self.n_hid) * 1e-1
        self.b1 = np.zeros(self.n_hid)
        self.W2 = np.random.randn(self.n_hid, n_out) * 1e-1
        ...
        if self.method == 'batch_gradient_descent':
            ...
            params = res.x
            W1, b1, W2, b2 = self.__unzip_params(params, n_in, self.n_hid, n_out)
    ...
    def __sigmoid__gradient(self, z):
    ...
    def __gradient(self, params, X, y, lambd, n_in, n_hid, n_out):
        ...
        dZ1 = dZ2 @ W2.T * self.__sigmoid__gradient(Z1)
        dW1 = (1 / m) * X.T @ dZ1
